...
void RunScheduling(const Nan::FunctionCallbackInfo<v8::Value>& info) {
    ...
    for (unsigned int i = 0; i < numItems; i++) {
        ...
        if (item->IsNumber()) {
            ...
            if (action == 0) {
                thread fcfsThread(runFCFS, ref(allProcessesA), ref(fcfsOrder), ref(fcfsDuration));
                fcfsThread.join();
            } else if (action == 1) {
                thread sjfThread(runSJF, ref(allProcessesB), ref(sjfOrder), ref(sjfDuration));
                sjfThread.join();
            } else if (action == 2) {
                thread psThread(runPS, ref(allProcessesC), ref(psOrder), ref(psDuration));
                psThread.join();
            } else if (action == 3) {
                thread rrThread(runRR, ref(allProcessesD), quantum, ref(rrTimeline), ref(rrDuration));
                rrThread.join();
            }
        }
    }
    ...
    // Prepare results to return to JavaScript
